...
async function fetchQuestions() {
    try {
        ...
        console.log(error);
        questionId.innerHTML = `<h5 style='color: red'>
        ${error}</h5>`;
    }
}
fetchQuestions();
...
if (Questions.length === 0) {
    questionId.innerHTML = `<h5>Please Wait!! 
...
function loadQues() {
    ...
    questionId.innerText = currentQuestion;
    optionsId.innerHTML = "";
    ...
    options.forEach((option) => {
        ...
        optionsId.appendChild(choicesdiv);
    });
}
...
setTimeout(() => {
    loadQues();
    if (Questions.length === 0) {
        questionId.innerHTML = `<h5 style='color: red'>Unable 
